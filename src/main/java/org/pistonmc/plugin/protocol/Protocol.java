...
public abstract class Protocol extends JavaPlugin {
    ...
    private static Map<ProtocolState, Map<Integer, Class<? extends Packet>>> packets;
    ...
    protected Protocol(int version, PlayerConnection connection) {
        ...
        for(ProtocolState state : ProtocolState.values()) {
            packets.put(state, new HashMap<Integer, Class<? extends Packet>>());
        }
    }
    ...
    public Packet create(ProtocolState state, int id) throws IllegalProtocolException {
